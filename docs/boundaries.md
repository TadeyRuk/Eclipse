...
| 2026-09-25 | Correct 2026-08-14 UI-overhaul rationale: L3 was not filed; it is in revision after a reviewer requested material product updates | The prior row incorrectly relied on stale local `docs/status.md`. L1 and L2 are filed; L3 still requires Preprod lifecycle evidence, final Lace demo, Idea #6 submission/approval, and Rise In resubmission. The dependency override remains a UI decision, not evidence that Gate 4 closed. |
| 2026-09-26 | Feature-owned web modules, injected SDK ports, executable import boundaries, and a reduced-motion-safe proof ceremony | Removes adapter leakage and duplicated lifecycle state while keeping the three-adapter cap and existing privacy/contract surface |
